...
echo ""
echo "  Configurar:"
echo "    nano .env                              # configurar tokens y modelo"
echo ""
echo "  Arrancar / Parar:"
echo "    systemctl start nexus-agent            # arrancar"
echo "    systemctl stop nexus-agent             # parar (para volver a local)"
echo "    systemctl restart nexus-agent          # reiniciar"
echo ""
echo "  Monitorear:"
echo "    systemctl status nexus-agent           # ver estado"
echo "    journalctl -u nexus-agent -f           # ver logs en vivo"
echo ""
echo "  Actualizar (cuando hagas git push desde tu PC):"
echo "    cd ~/ai-agent-framework && git pull && source venv/bin/activate && pip install -q -r requirements.txt && systemctl restart nexus-agent"
echo ""
